...
# Check the column names and first few rows to confirm correct data
colnames(covid_all_states)
head(covid_all_states)

# Example population data (replace with actual population data for more states)
...
# Optionally, you can print the first few rows to check the final dataset
...
county_centroids <- counties %>%
  left_join(county_centroids_joined, by = "fips")  # Assuming centroids has LAT and LON
# Select LAT and LON columns from the correct dataframe
county_centroids %>%
  select(fips, LAT, LON)
...
exists("covid_data_filtered")

# Select only the necessary columns from covid_data_aggregated and counties
covid_data_aggregated <- covid_data_aggregated %>% select(fips, date, total_cases, total_deaths)
counties <- counties %>% select(fips, LAT, LON)

setDT(covid_data_aggregated)
setDT(counties)

# Then, perform the merge
merged_data <- merge(covid_data_aggregated, counties, by = "fips", all.x = TRUE)

covid_data_with_centroids <- merge(covid_data_aggregated, counties, by = "fips", all.x = TRUE)

# Filter the covid data for the desired date range
covid_data_filtered <- covid_data %>%
  filter(date >= "2020-01-01" & date <= "2021-01-01")

# Convert the filtered data to data.table
setDT(covid_data_filtered)

# Filter counties based on 'fips' codes present in filtered covid data
counties_filtered <- counties %>%
  filter(fips %in% unique(covid_data_filtered$fips))

# Ensure the 'date' column is in Date format
covid_data$date <- as.Date(covid_data$date, format = "%Y-%m-%d")

# Convert 'fips' column in both data frames to character type for proper join
covid_data$fips <- as.character(covid_data$fips)
counties$fips <- as.character(counties$fips)

# Merge COVID-19 data with county centroids using 'fips' code
# Assuming you want to join covid_data with county_centroids_joined
county_centroids <- covid_data %>%
  left_join(county_centroids%>% select(fips, LAT, LON), by = "fips")

setDT(covid_data)
setDT(counties)

county_centroids <- covid_data[counties, on = "fips"]

# Calculate the weighted mean center for daily deaths
covid_weighted_center_deaths <- county_centroids %>%
  group_by(date) %>%
  summarise(
    total_deaths = sum(deaths, na.rm = TRUE),
...
covid_weighted_center_cases <- county_centroids %>%
  group_by(date) %>%
  summarise(
    total_cases = sum(cases, na.rm = TRUE),
